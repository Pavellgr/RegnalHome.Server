...
namespace RegnalHome.Server.Grpc.ClientFactories
{
    public abstract class GrpcClientFactory<T> where T : ClientBase
    {
        ...
        public abstract Task<Therm.ThermClient> CreateClient(string address);

        public abstract Task<IEnumerable<(string Address, Therm.ThermClient Client)>> CreateClients();
